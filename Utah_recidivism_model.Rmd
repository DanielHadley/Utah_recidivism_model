...
In the latest simulation of 1,000 agents, the average total cost per parolee per year was `r dollar(round(mean(multi_agent_test$avg_total_cost_per_yr)))`. The standard deviation was `r dollar(round(sd(multi_agent_test$avg_total_cost_per_yr)))`. Here is a histogram of the distribution of the average total cost per year. Some will cost the state very little, while others will cost tens of thousands of dollars:
...
The maximum cost in this simulation was `r dollar(round(max(multi_agent_test$avg_total_cost_per_yr)))`. This would likely be someone who bounced in and out of prison very rapidly. On the other end are parolees who cost the state less than `r dollar(round(quantile((multi_agent_test$avg_total_cost_per_yr), .25)))`, which is the bottom quartile of annual costs. This group likely stayed out of prison, or only went back for a brief time period. Their costs are more associated with parole, health benefits, and shelters, in certain cases.

Prison remains the largest cost factor for parolees. The average simulation resulted in prison costs of `r dollar(round(max(multi_agent_test$avg_prison_cost_per_yr)))` per year, or `r percent(mean(multi_agent_test$avg_prison_cost_per_yr) / mean(multi_agent_test$avg_total_cost_per_yr))` of the total cost. 
