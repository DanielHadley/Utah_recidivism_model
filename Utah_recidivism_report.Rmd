...
#
# http://cad.sagepub.com/content/early/2014/09/26/0011128714549655.abstract
#
...
### The costs of crime
# http://www.justice.utah.gov/Documents/CCJJ/Cost%20of%20Crime/Utah%20Cost%20of%20Crime%202012%20-%20Methods%20Review%20Cost.pdf
# TODO: add defense attourneys, who were left out of the report above
# courts
...
sample_a_combined_cost<-function(parole_cost_type){
...
calc_combined_cost <- function(combined_cost_sample, m.is_in_prison){
  
  if_else(m.is_in_prison == 1, 0, combined_cost_sample)
...
sim_single_agent <- function(months, parole_cost_type) {
  ...
  # We will add this in the for loop, but only want to sample once
  combined_cost_sample <- sample_a_combined_cost(parole_cost_type)
  ...
  for (month in 1:months) {
    ...
    # Combined costs (Emergency room & DWS+DHS & Shelters)
    m.total_combined_costs <- calc_combined_cost(combined_cost_sample, m.is_in_prison)
...
sim_multi_agents <- function(n_agents, n_months,parole_cost_type){
  ...
  df <- data.frame(prison_time=numeric(0),
                   ...
                   avg_court_costs_per_yr=numeric(0),
                   avg_policing_costs_per_yr=numeric(0))
  ...
  for (agent in 1:n_agents) {
    ...
    by_year <- single_agent %>% 
      ...
      summarise(total_prison_costs = sum(as.numeric(total_prison_costs)),
                ...
                total_court_costs = sum(as.numeric(total_court_costs)),
                total_policing_costs = sum(as.numeric(total_policing_costs))) %>% 
    ...
    a.months_free <- n_months - a.prison_time
    ...
    a.avg_court_costs_per_yr <- mean(by_year$total_court_costs)
    a.avg_policing_costs_per_yr <- mean(by_year$total_policing_costs)
    
    
    # add month to the data frame
    df[agent,] <- 
      c(a.prison_time, a.avg_prison_time_per_yr, a.months_free, 
        a.total_prison_costs, a.avg_prison_cost_per_yr, 
        a.median_prison_cost_per_yr, a.prison_cost_yr_one,
        a.arrests, a.total_costs, a.avg_total_cost_per_yr, 
        a.median_total_cost_per_yr, a.avg_other_cost_per_yr,
        a.avg_combined_cost_per_yr, a.avg_parole_cost_per_yr,
        a.avg_court_costs_per_yr, a.avg_policing_costs_per_yr)
    
  }
  
  df <- df %>% 
    mutate(avg_law_enforcement = avg_prison_cost_per_yr +
             avg_parole_cost_per_yr + avg_court_costs_per_yr + 
             avg_policing_costs_per_yr,
           avg_non_law_enforcement = avg_combined_cost_per_yr)
...
colnames(table)<-c("Model","Non Law Enforcement Costs")
kable(head(table,10),digits=4,align = 'c')
...
for (i in sensitivity_recid_reductions) {
  ...
  multi_agent_post <- sim_multi_agents(n_agents = 100, n_months = n_months,"base")
