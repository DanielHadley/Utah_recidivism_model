...
This table above shows that a parolee will end up accruing over twice the cost as he would in prison. This can be explained by the amount of time spent in prison compared to the time spend on parole. In the following table it shows that with increasing recidvism reductions how the average months in prison decreases. 
...
c(round(mean(multi_agent$prison_time*30.5/12)),
                    round(mean(multi_agent_low$prison_time*30.5/12)),
                    round(mean(multi_agent_med$prison_time*30.5/12)),
                    round(mean(multi_agent_high$prison_time*30.5/12))),

c(round(median(multi_agent$prison_time*30.5/12)),
                    round(median(multi_agent_low$prison_time*30.5/12)),
                    round(median(multi_agent_med$prison_time*30.5/12)),
                    round(median(multi_agent_high$prison_time*30.5/12)))
)
...
This table also shows how little time is spent in prison with an average of `r round(mean(multi_agent$prison_time)*30.5/12)` days in prison per year, supporting our higher out of prison cost. 

These tables do not fully show the effect the super-utilizers have on cost. It is reasonable to assume that with super utilizers there will be a discernable gap between the average and mean cost per individual because of the weight of their skew. 
