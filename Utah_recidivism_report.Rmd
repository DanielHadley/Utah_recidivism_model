...
* The average parole sentence will go from three years to `r parole_length_JRI` months for those who exhibit good behavior, as per [this](http://law.justia.com/codes/utah/2006/title76/76_03009.html) document. 
...
* DHS costs include some inpatient and outpatient care (e.g., methadone). I also assume that the average parolee will visit the ER two times per year, while free, at an average cost of `r dollar(round(ambulance + ER))` per trip.  
...
### Single Agent 
When run for a single person, the resulting data contain `r n_months / 12` rows, each representing a month in the life of the simulated parolee. The columns track outcomes of the simulation and their associated costs: `r names(single_agent_test)`. 
...
The power of this model comes from tracking the outcomes of multiple simulations. For each one, we track time in and out of prison and the associated costs. This allows us to answer the question of what would happen if we recuded recidivsm among the COD population. To help visualize this, here is a matrix of 12 simulations using the same chart format from the previous page:  
...
Instead of 12, however, the final model will peform `r comma(n_agents)` simulations, summing prison time, parole, and all of the costs measured for each individual simulation.  
...
## Description:
These are the results of running the same model as above, but assuming a recidivism reduction of `r percent(recid_reduction_post_optimistic)` instead of `r percent(recid_reduction_post)`. I also examine some of the other costs, and what would happend if those were reduced. 
