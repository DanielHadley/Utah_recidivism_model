...
In the latest simulation of 1,000 agents, the average total cost per parolee per year was **`r dollar(round(mean(multi_agent_test$avg_total_cost_per_yr)))`**, with a standard deviation of `r dollar(round(sd(multi_agent_test$avg_total_cost_per_yr)))`. The median was `r dollar(round(median(multi_agent_test$avg_total_cost_per_yr)))`. Here is a histogram of the distribution of the average total cost per year. Some will cost the state very little, while others will cost tens of thousands of dollars:
...
```{r cost2, echo=FALSE}
# plot the hist
ggplot(multi_agent_test, aes(avg_prison_cost_per_yr)) + 
  geom_histogram(fill="#c0392b", alpha=0.75, color = "white", bins = 30) +
  ylab("# of Simulations") +
  xlab("Average Total Cost Per Year: Prison") +
  scale_x_continuous(labels=dollar)
```

```{r, echo=FALSE}
more_than_two <- multi_agent_test %>% 
  filter(arrests > 2) %>% 
  nrow()
```

This is because, even accounting for JRI, more than one half of our COD population will return to prison within 5 years. In fact, many will return multiple times. In these simulations, `r percent(more_than_two / nrow(multi_agent_test))` returned more than twice:

```{r returns, echo=FALSE}
# plot the hist
ggplot(multi_agent_test, aes(arrests)) + 
...
### With Treatment
```{r, echo=FALSE}
treatment_recid_reduction <- .15

survival_rates <- load_survival_data(recid_reduction_ovr_cntrl = treatment_recid_reduction)

multi_agent_test <- sim_multi_agents(1000, 60)

```

Assuming a recidivism reduction of `r percent(treatment_recid_reduction)`, instead of approximately $14,000, the average parolee will cost the state of Utah **`r dollar(round(mean(multi_agent_test$avg_total_cost_per_yr)))`**. 