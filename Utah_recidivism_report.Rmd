...
The challenge is determining how much time the average person from the COD demographic is expected to spend in prison, once released, and what reducing that is likely to save the State. This is a difficult prediction problem, given all of the factors; but we have created a model that does a reasonable job of simulating a person's chances of returning to prison. 


### Summary
By reducing recidivism among the COD population by 15%, the State can expect savings in prison expenditures on the order of $2,000 per parolee per year. These savings would come from direct reductions in prison time for the average parolee. By providing a higher level of care for the COD population, the State can also expect to see ancillary benefits in the form of reduced crime, shelter days, and outpatient stays, for example. These are more difficult to quantify, but we conservatively estimate the potential savings at $1,500 per parolee per year. 

There are several other indirect benefits, which we do not explore in this report, e.g., reductions in victim costs, or improvements in job prospects for the COD group. We hope that a pilot program will help to better define the potential costs and benefits of proiding intense support to this population. 
...
## Assumptions:
* The crime type frequencies for a reoffense will be similar to those from the original offense.
...
prob_of_ER_visit <- .16  # two per year - your chance per month
ambulance <- 1200
ER <- 1200

calc_total_ER_costs <- function(m.is_in_prison){
  
  did_visit <- ifelse(m.is_in_prison == 1, 0,
                      sample(x = (c(1,0)), 1, 
                             prob = c(prob_of_ER_visit, 1 - prob_of_ER_visit)))
  
  cost <- ifelse(did_visit == 1, ambulance + ER, 0)
...
sim_single_agent <- function(months) {
  ...
  for (month in 1:months) {
    ...
    # Emergency room
    m.total_ER_costs <- calc_total_ER_costs(m.is_in_prison)
    ...
    df[month,] <- 
      c(m.month, m.year, m.months_free, m.rearrested_or_not, m.prison_sentence, 
        ...
        m.total_parole_costs, m.total_DWS_DHS_costs, m.total_ER_costs)
...
## Assumptions:
* It is advisable to look at average costs over the course of `r n_months / 12` years with a sample of at least `r n_agents` simulated agents. Shorter time frames with fewer simulations may lead to anomalous results.
...
```{r, echo=FALSE}
survival_rates <- load_survival_data(recid_reduction_ovr_cntrl = 0)
multi_agent <- sim_multi_agents(n_agents = n_agents, n_months = n_months)
```
...
##### Pagebreak
The following histogram shows the average annual costs for the simulated COD population. The x-axis shows annual costs, starting at `r dollar(round(min(multi_agent$avg_total_cost_per_yr)))` and ending in the maximum, `r dollar(round(max(multi_agent$avg_total_cost_per_yr)))`. The y-axis shows the number of simulations that resulted in the average cost represented on the x-axis. The graph has two distinct "peaks." The first one appears to represent parolees who did not return to prison, or only went back briefly. The second group likely went back for the average amount of time, which for the entire `r n_months/12` year simulation was `r round(mean(multi_agent$prison_time))` months.   

```{r cost, echo=FALSE}
# plot the hist
ggplot(multi_agent, aes(avg_total_cost_per_yr)) + 
  geom_histogram(fill="#c0392b", alpha=0.75, color = "white", bins = 30) +
  labs(title = "Histogram of Annual Costs", y = "# of Simulations", x = "Average Total Cost Per Year") +
...
multi_agent_post <- sim_multi_agents(n_agents = n_agents, n_months = n_months)
```
...
```{r, echo=FALSE}
recid_reduction_post_optimistic <- .25
survival_rates <- load_survival_data(recid_reduction_ovr_cntrl = recid_reduction_post_optimistic)
multi_agent_post_o <- sim_multi_agents(n_agents = n_agents, n_months = n_months)
```


## Description:
These are the results of running the same model as above, but assuming a recidivism reduction of `r percent(recid_reduction_post_optimistic)` instead of `r percent(recid_reduction_post)`. I also examine some of the other costs, and what would happen if those were reduced. 



```{r, echo=FALSE}
y1 <- mean(multi_agent$avg_prison_cost_per_yr)
y2 <- mean(multi_agent_post_o$avg_prison_cost_per_yr)
```
## Statistics:
* Average annual prison costs are reduced from `r dollar(round(y1))` to `r dollar(round(y2))`, which is a reduction of `r dollar(round(y1 - y2))`, or `r percent((y2 - y1) / y1)`.
* The average number of prison days per year for this group was `r round(mean(multi_agent_post_o$avg_prison_time_per_yr) * 30.5)`, with a standard deviation of `r round(sd(multi_agent$avg_prison_time_per_yr) * 30.5)`.

## Other Assumptions
So far, this report has examined only the cost savings of reductions in prison time. Presumably, by providing a higher level of care for the COD population, we will also expect to see ancillary benefits. Returning to the more conservative scenario, where recidivism is reduced by `r percent(recid_reduction_post)`, we might expect to see other costs reduced.

* For example, if this treatment reduced costs other than ones associated with prison by `r percent(recid_reduction_post)`, there would be additional annual savings of `r dollar(round(recid_reduction_post * mean(multi_agent_post$avg_other_cost_per_yr)))` per parolee.